import { callUploadFileImgAPI, createBookAPI, getCategoryAPI, updateBookAPI } from '@/services/api';
...
const FormUpdateBook = (props: IProps) => {
    const { refreshTable, isModalUpdateOpen, setIsModalUpdateOpen, dataUpdate, setDataUpdate } = props;
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = async (values) => {
        ...
        const res = await updateBookAPI(data, dataUpdate?._id);
        if (res && res.data) {
    ...
    useEffect(() => {
        ...
        const thumbnailUpdate: CommonFileType[] = thumbnailImg.map(it => ({
            uid: it.uid,
            name: it.name,
            status: 'done',
            url: it?.url,
        ...
        setFileListThumbnail(thumbnailUpdate);

        const sliderImg: UploadFile[] = dataUpdate.slider.map(name => ({
            uid: v4(),
            name,
            status: 'done',
        ...
        const sliderUpdate: CommonFileType[] = sliderImg.map(it => ({
            uid: it.uid,
            name: it.name,
            status: 'done',
            url: it.url,
        ...
        setFileListSlider(sliderUpdate);
